...
function getComponents(components) {
	return (nextState, callback) => {
		...
		const componentRoutes = componentNames.map(
		...
		Promise.all(
			componentRoutes.map((route) =>
				System.import(`../${route}/index.jsx`)
			)
		) // Webpack v2 import.
			.then((resolvedRoutes) => {
				const objectResolvedComponents = resolvedRoutes.reduce(
					(result, current, index) => {
						Object.assign(result, {
							[componentNames[index]]: current.default || current,
						});
						return result;
					},
					{}
	...
	<Route path="/" component={App}>
		<IndexRoute
			getComponents={getComponents({
				...
				header: "components/TransparentHeader",
			})}
		/>
		<Route
			path="/rooms"
			getComponents={getComponents({
				main: "containers/Rooms",
				header: "containers/DefaultHeader",
			})}
		/>
		<Route
			path="/rooms/new"
			getComponents={getComponents({
				main: "containers/CreateRoom",
				header: "containers/DefaultHeader",
			})}
		/>
		<Route
			path="/room/:slug"
			getComponents={getComponents({ main: "containers/RoomBase" })}
		/>
		<Route
			path="/room/:slug/:chat"
			getComponents={getComponents({ main: "containers/RoomBase" })}
